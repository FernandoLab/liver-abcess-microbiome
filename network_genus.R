...
#                       row.names = 1,
#                      sep = ",")
la_genus_ps <- tax_glom(la_ps2, taxrank = 'Genus', NArm = T)
asv.table<- as(otu_table(la_genus_ps),'matrix')
class(asv.table)

#change the asv consensus number to taxonomy name
colnames(asv.table) <- tax_table(la_genus_ps)[,"Genus"]

amgut1.filt <- asv.table
...
#colnames(SpiecEasi.matrix$refit$stars) <- colnames(amgut1.filt)

#head(SpiecEasi.matrix$refit)
# Build network from adjacency
...
#asv1_neighbors <- neighbors(net, "ASV1")
#asv114_neighbors <- neighbors(net, "ASV114")
# Find neighbors shared by nodes 1 and 25
#intersection(asv1_neighbors, asv114_neighbors)


# Edges incident to OTU_1
#asv1.edges <- incident(net, "ASV1", mode = "all")
...
clean.net <- delete.vertices(net, which(degree(net, mode = "all")
                                        == 0))
node.names <- V(clean.net)$name
num.nodes <- vcount(clean.net)
num.nodes
num.edges <- ecount(clean.net)
num.edges
...
#which(names(net.comps$membership) == "OTU_1")]
...
pdf("/Volumes/Wes_External/LA_microbiome/sparcc_net_genus.pdf", width = 35, height = 30 )
plot(wt,net)
dev.off()
...
plot.net <- function(net, scores, outfile, title) {
  ...
  outfile <- paste(outfile, "tiff", sep=".")
  # Image properties.
...
#plot.net(net, net.hs, outfile = "network_la_phylum", title = "SparCC Network: Phylum")
...
plot.net.cls <- function(net, scores, cls, AP, outfile, title) {
  ...
  # Image properties.
  jpeg(outfile, width = 7200, height = 8700, res = 300, quality = 100)
  par(oma = c(4, 1, 1, 1))
...
#plot.net.cls(net, net.hs, wt, AP, outfile = "sparcc_la_network2_genus", title = "SparCC Network: Genus")


# Convert node label from names to numerical IDs.
features <- V(clean.net)$name
col_ids <- seq(1, length(features))
V(clean.net)$name <- col_ids
node.names <- features[V(clean.net)]
wt <- walktrap.community(clean.net)
l <- layout_nicely(clean.net)

quartzFonts(times = c("Times New Roman Regular", "Times New Roman Bold", "Times New Roman Italic", 
                        "Times New Roman Bold Italic"))


pdf("/Users/westom/Desktop/sparcc_genus_net.pdf", width =25, height = 20)
par(family='serif')
plot(wt,clean.net,layout=l, vertex.label.cex=2.5)
title("SparCC Network: Genus", cex.main=4)
labels = paste(as.character(V(clean.net)), node.names, sep = ") ")
legend("topleft", legend = labels, xpd = TRUE, ncol = 2, cex = 1.5, bty = 'n')
dev.off()
